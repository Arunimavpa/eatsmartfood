...
// Check if penalty ID (repId) is provided
if (!isset($_GET['repId'])) {
    echo '<script>alert("No penalty record found!"); window.location.href="restaurant_penalty.php";</script>';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_payment'])) {
    ...
    // Simulate successful payment and update status
    if ($payment_method == 'upi') {
        $upi_id = $_POST['upi_id'];
        $updateQuery = "UPDATE tblpenalty SET status = 'Paid' WHERE repId = ?";
        $stmt = $conn->prepare($updateQuery);
        $stmt->bind_param("i", $repId);

        if ($stmt->execute()) {
            echo '<script>alert("UPI Payment Successful! UPI ID: ' . $upi_id . '"); window.location.href="restaurantpenalty.php?repId=' . $repId . '";</script>';
        } else {
            echo '<script>alert("Error updating penalty status. Please try again."); window.location.href="restaurantpenalty.php?repId=' . $repId . '";</script>';
        }
        ...
        if ($stmt->execute()) {
            echo '<script>alert("Card Payment Successful!"); window.location.href="restaurantpenalty.php?repId=' . $repId . '";</script>';
        } else {
            echo '<script>alert("Error updating penalty status. Please try again."); window.location.href="restaurantpenalty.php?repId=' . $repId . '";</script>';
        }
    } else {
        echo '<script>alert("Payment failed! Try again."); window.location.href="pay_fine.php";</script>';
    }
    $stmt->close();
    $conn->close();
}
...
<body>
    <div class="container">
        <h2>Penalty Payment Page</h2>
        <form method="POST" id="paymentForm">
            <input type="hidden" name="repId" value="<?php echo htmlspecialchars($repId); ?>">
